// Unitaire — StashView : le formulaire de laine écrit et propose des lignes d'achat
...
// Modèle : tests/unit/StashView-form-units.spec.js (même approche base réelle).
...
function todayISO() {
  // Même calcul que l'implémentation (StashView.vue:todayISO), qui est passée en heure LOCALE
  // le 21/08. Écrit en UTC ici, ce helper figeait la mauvaise référence : il ne
  // l'aurait montré qu'en tournant entre minuit et 02 h, heure française.
  return ymdLocal(new Date())
}

async function mountStash({ yarns = [], purchases = [], loadPurchases = false } = {}) {
  ...
  // En production, App.vue charge le store `purchases` au démarrage, AVANT que StashView
  // ne monte (cf. YarnPurchases.vue). Ici, StashView est monté seul : sans ce chargement
  // explicite, `<YarnPurchases>` (embarqué en édition) ne verrait JAMAIS les lignes déjà en
  // base — `forYarn()` lirait un store resté vide, et le bloc « Achats et cadeaux » (gap,
  ...
  // cf. StashView-form-units.spec.js : settings.load() enchaîne des lectures Dexie
...
async function openEditOn(w, name) {
...
async function openDuplicateOn(w, name) {
  const card = w.findAll('.ycard').find((c) => c.text().includes(name))
  await card.find('.ycard__kebab').trigger('click')
  await flushPromises()
...
// jusqu'au bout (même piège de fond que la boucle d'attente de `settings.loaded` dans
// StashView-form-units.spec.js, mesuré empiriquement ici avant d'écrire ce commentaire).
...
describe('StashView — formulaire de laine et registre d’achats', () => {
  beforeEach(async () => {
    await db.open()
  })
  ...
  it('C1 : « Ajuster le stock » à 0 écrit VRAIMENT 0 en base (pas 1), et fait disparaître l’alerte', async () => {
    ...
    await openEditOn(w, 'Cheval Blanc')
    ...
    // Le formulaire se ferme après une baisse (pas de proposition) : le rouvrir pour
    // vérifier que l'alerte a bien disparu (stock 0 = historique 0, écart nul) — pas
    // juste que la base contient la bonne valeur.
    await openEditOn(w, 'Cheval Blanc')
  ...
  it('comportement 1c : en ÉDITION, les champs « date d’achat » et « bain » de la création ne sont PAS rendus', async () => {
    ...
    await openEditOn(w, 'Schachenmayr')
  ...
  it('comportement 1d : en DUPLICATION, les champs sont présents, date du jour, bain vide (jamais celui de la fiche source)', async () => {
    ...
    await openDuplicateOn(w, 'Bergère de France')
    const dateField = w.find('#yarn-purchased-at')
    const bainField = w.find('#yarn-bain')
    expect(dateField.exists()).toBe(true)
    expect(bainField.exists()).toBe(true)
    expect(dateField.element.value).toBe(todayISO())
    expect(bainField.element.value).toBe('') // jamais 'SOURCE-BAIN' : décision déjà prise (cf. openDuplicate)
  })
  ...
  it('comportement 6 : openDuplicate ne recopie AUCUNE ligne d’achat de la fiche source', async () => {
    const SOURCE = { id: 5, brand: 'Drops', model: 'Baby Merino', colorName: 'Bleu', quantity: 5, composition: [], price: '20,00' }
    await db.yarns.clear()
    await db.settings.clear()
    await db.purchases.clear()
    await db.yarns.add(SOURCE)
    ...
    const router = createRouter({ history: createMemoryHistory(), routes: [{ path: '/', name: 'stash', component: StashView }] })
    router.push('/')
    await router.isReady()
    const pinia = createPinia()
    const w = mount(StashView, { global: { plugins: [router, i18n, pinia] } })
    const settings = useSettingsStore(pinia)
    let rounds = 0
    while (!settings.loaded && rounds < 20) { await flushPromises(); rounds++ }
    await flushPromises()

    await openDuplicateOn(w, 'Drops')
    await w.find('#yarn-color-name').setValue('Verte') // requis : openDuplicate vide colorName
    await clickSave(w)
    ...
    // La ligne fraîche suit les valeurs REMISES À ZÉRO par openDuplicate (quantité 1, prix
  ...
  it('verrou (ordre) : à la création, le formulaire est ENCORE affiché au moment où l’écriture de l’achat démarre', async () => {
    ...
    // checkbox.spec.js. L'invariant est donc renversé : LE PANNEAU REFERMÉ SIGNIFIE
    // « TOUT EST ÉCRIT ».
    ...
    // que le panneau tient bon pendant tout ce temps — vit dans
    // tests/unit/stash-fermeture-apres-ecriture.spec.js, qui couvre aussi le chemin ÉDITION.
    const { w, pinia } = await mountStash()
    await openAddForm(w)
    ...
    let formWasOpenBeforeWrite = null
    ...
    vi.spyOn(purchasesStore, 'add').mockImplementation(async (data) => {
      ...
      await nextTick()
      formWasOpenBeforeWrite = w.find('.addform').exists()
    ...
    expect(formWasOpenBeforeWrite).toBe(true) // le panneau n'annonce rien avant d'avoir fini
    ...
    expect(w.find('.addform').exists()).toBe(false) // …le panneau ne se fermant qu'après
  })
  ...
  it('« Ajuster le stock » (réconciliation avec l’historique) ne propose PAS un nouvel achat — pas de double comptage', async () => {
    ...
    const { w } = await mountStash({
      ...
      loadPurchases: true, // sans ça, <YarnPurchases> ne verrait jamais la ligne (cf. mountStash)
    })
    await openEditOn(w, 'Bergère')
    await w.find('[data-test="purchases-correct"]').trigger('click')
    await flushPromises()
    await w.find('[data-test="purchases-correct-adjust"]').trigger('click')
    await flushPromises()
