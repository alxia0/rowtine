...
![The full-screen view where you line the chart up: the top bar reads Chart · 8 sts × 24 rows beside the align, zoom and close icons, two lines carry the labels Top of the grid and Bottom of the grid a few rows apart, and Reset, Cancel and Save sit at the bottom](images/en/03c-diagramme-calage.webp)
...
![The calibrated granny chart, full screen, with deliberately blurred stitches: the first two finished rounds are circled by a discreet ring from the central flower, the third round, the current one, stands out in a thicker ring, the outer boundary of the calibration frames it all in dashes, and the bottom bar reads 3 of 5 between a minus and a plus button, followed by the curtain flip](images/en/03i-granny-cale.webp)
...
![The current row ringed, the round double-arrow handle sitting mid-row, the finished rows tinted below it, and 7 / 24 along the bottom with the button that flips the tinted side](images/en/03d-diagramme-rideau.webp)
...
![The All (3), Clothing (0) and Accessories (3) chips with a fourth one cut off by the right edge, the full-width Add pattern button, then Granny Bag in crochet, Cloud Scarf in knitting and Cable Beanie, the only one carrying its S, M, L sizes](images/en/04-bibliotheque.webp)
...
![Under the Fix the pattern title, the Gauge block reads 20 sts × 26 rounds in stockinette, 4.5mm needles = 10 × 10 cm, then the Yarn block, the Needles/Hook heading and the Cancel and Save bar](images/en/04c-correction-editeur.webp)
...
![The Counter name field still empty, both boxes unticked beside their examples (a sleeve, or repeat rows 1 to 8, 5 times), then the + Counter button and the line No counters yet. Add one.](images/en/06a-compteurs.webp)

*The free counter, usable without a project: name it, and choose whether it tracks increases/decreases or repeats.*

![Increases, Flat and Distribution picked out, then the empty Current stitches, Target stitches, Increases each time and Over how many rows fields, hinting 60, 80, 1 and 40](images/en/06b-calculateur.webp)
...
![Find my needle size with the fields filled in: inches is the unit picked, Yarn label gauge holds 4 mm, 22 stitches and 30 rows per 4×4 in, Target gauge 20 and 28, and the answer Try a size up: 4.5 mm](images/en/06c-aiguilles.webp)
...
![Camille typed into "Your first name (optional)", Knitting picked out as the default craft and a Save button closing off the Profile card, then Appearance on System, the Ambience color block with its ten dots and its hue slider, First day of the week on Monday, and the Language heading alone at the bottom edge](images/en/08-reglages.webp)
